...
-- absolute paths of the entries the library move placed there for it (every
-- version file, sidecar and subfolder). A delete and a re-download act on
-- exactly these, so two lessons whose names look alike can never take each
-- other's files.
