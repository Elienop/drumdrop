...
func TestPlexTVMoveRecordsExactlyWhatItPlaced(t *testing.T) {
	...
	res, err := testMovePlexTV(t, lib, plexEpisode{"Songs", 1, 5, "Even Flow"}, lessonDir, plexLibrary{}, []database.Lesson{other}...)
	if err != nil {
		t.Fatalf("move: %v", err)
	}
	want := paths(season,
...
func TestPlexTVMoveAcceptsAnyName(t *testing.T) {
	...
	res, err := testMovePlexTV(t, lib, plexEpisode{"Drum Fills [Beginner]", 1, 5, "Fill[1]"}, lessonDir, plexLibrary{})
...
func TestMoveToLibraryPlexTVFolderCopyFailsPartWayIsRemoved(t *testing.T) {
	...
	res, err := testMovePlexTV(t, filepath.Join(tmp, "lib"), plexEpisode{"Songs", 1, 5, "Even Flow"}, lessonDir, plexLibrary{})
...
func TestPlexTVMoveFitsLongNames(t *testing.T) {
	...
	res, err := testMovePlexTV(t, lib, plexEpisode{show, 1, 5, title}, lessonDir, plexLibrary{})
	if err != nil || res.seasonDir == "" {
		t.Fatalf("move = (%+v, %v), want a move", res, err)
	}
	...
	res, err = testMovePlexTV(t, filepath.Join(tmp2, "lib"), plexEpisode{strings.Repeat("é", 150), 1, 5, "T"}, lessonDir, plexLibrary{})
	if err == nil || res.seasonDir != "" {
...
func TestCopyIsFlushedBeforeTheSourceGoes(t *testing.T) {
	...
	res, err = testMovePlexTV(t, filepath.Join(tmp2, "lib"), plexEpisode{"Songs", 1, 5, "Even Flow"}, lessonDir2, plexLibrary{})
	if err == nil || res.seasonDir != "" {
