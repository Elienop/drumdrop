...
func TestMoveRefusesALessonFolderThatIsASymlinkInsideDownloads(t *testing.T) {
	...
	for _, layout := range []string{LayoutPlexTV, ""} {
		t.Run("layout="+layout, func(t *testing.T) {
			checkMoveRefusesASymlinkedLessonFolder(t, layout)
		})
	}
}

// checkMoveRefusesASymlinkedLessonFolder is
// TestMoveRefusesALessonFolderThatIsASymlinkInsideDownloads in layout.
func checkMoveRefusesASymlinkedLessonFolder(t *testing.T, layout string) {
	t.Helper()
	tmp := t.TempDir()
	...
	moved, err := placeFiveIn(t, layout, dl, lib, lessonDir, plexLibrary{}, database.Lesson{})
	if moved != "" {
		t.Errorf("placed in %q, want none", moved)
	}
	if err == nil {
