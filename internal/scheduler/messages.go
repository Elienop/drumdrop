...
const (
	...
	// msgLeftBehind is failLeftBehind's sentence under the lesson. The
	// placement failed for a reason written only to the log; the old folder
	// is why it didn't fall back to downloads, so the sentence says both
	// facts without making one the cause of the other. It offers only the
	// move: switching back is safe only while the new folder is empty, which
	// doesn't fit in the Lessons row's two lines (see
