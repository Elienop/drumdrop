...
func TestCancelJobQueued(t *testing.T) {
	...
	id, err := store.EnqueueJob(ctx, sql.NullInt64{}, 6001)
	if err != nil {
...
func TestCancelJobTerminalConflict(t *testing.T) {
	...
	id, err := store.EnqueueJob(ctx, sql.NullInt64{}, 6002)
	if err != nil {
		t.Fatalf("EnqueueJob: %v", err)
	}
...
func TestRetryJobFailed(t *testing.T) {
	...
	id, err := store.EnqueueJob(ctx, sql.NullInt64{}, 6003)
	if err != nil {
		t.Fatalf("EnqueueJob: %v", err)
	}
	if err := store.MarkJobRunning(ctx, id); err != nil {
...
func TestRetryJobNotTerminalConflict(t *testing.T) {
	...
	id, err := store.EnqueueJob(ctx, sql.NullInt64{}, 6004)
	if err != nil {
		t.Fatalf("EnqueueJob: %v", err)
	}
	srv := NewServer(store, Deps{}, nil, Config{}, "test")
